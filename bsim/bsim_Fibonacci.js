...
`.include "beta.uasm"
   ...
   LD(a,R1)   // On charge le contenue de a dans le registre R1
   LD(b,R2)   // On charge le contenue de b dans le registre R2
   LD(n,R4)   // On charge le contenue de n dans le registre R4
loop:
   ADD(R1,R2,R3)    // R3 <- R1 + R2
   MOVE(R2,R1)      // R1 <- R2
   MOVE(R3,R2)      // R2 <- R3
   SUBC(R4,1,R4)    // R4 <- R4 - 1
test:
   CMPLT(R31,R4,R5) // If (R31 < R4) Then R5 <- 1 Else R5 <- 0
   BT(R5,loop)      // If R5!=0 Then GoTo loop
 
   ST(R3,c)   // Charge le contenu du registre R3 à l'adresse c
   HALT()     // On arrête l'execution
 
a: LONG(0) // On initialise a (1e terme = 0)
b: LONG(1) // On initialise b (2e terme = 1)
c: LONG(0) // adresse dans laquelle on va retourner le resultat
n: LONG(5) // On initialise n (terme que l'on souhaite calculer)  
